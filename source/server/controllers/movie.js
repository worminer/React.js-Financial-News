...
module.exports = {
      ...
      Movie.create(movieData).then(movie => {
        if (!movie) {
          return res.status(500).
            send({message: 'Cannot write movie in database'})
        }
  ...
  topTen: {
    get: (req, res) => {
      Movie.find().sort({score: -1}).limit(10).then(movies => {
        if (!movies) {
          return res.status(400).
            send({message: 'No movies. Care to add some?'})
        }
  ...
  fiveRecent: {
    get: (req, res) => {
      Movie.find().sort({dateCreated: -1}).limit(5).then(movies => {
        if (!movies) {
      ...
      Movie.find(
        ...
        if (movies.length === 0) {
          return res.status(404).
            send({message: 'Sorry! Movie not found...'})
        }
